...
/**
 * Get sorted locales with primary locales first
 */
export function getSortedLocales(): string[] {
  const primarySet = new Set(PRIMARY_LOCALES);
  const others = ALL_LOCALES.filter(code => !primarySet.has(code)).sort();
  return [...PRIMARY_LOCALES, ...others];
}

/**
 * Get locales grouped by language (base language code)
 */
export function getLocalesByLanguage(): Map<string, string[]> {
  ...
  for (const code of ALL_LOCALES) {
    ...
    grouped.get(lang)!.push(code);
  }
  return grouped;
}

/**
 * Get locales grouped by family
 */
export function getLocalesByFamily(): Map<LanguageFamily, string[]> {
  const grouped = new Map<LanguageFamily, string[]>();
  for (const code of ALL_LOCALES) {
    const family = LOCALE_FAMILIES[code] || 'other';
    if (!grouped.has(family)) {
      grouped.set(family, []);
    }
    grouped.get(family)!.push(code);
  }
  return grouped;
}
 ...
 */
export function getLocalesByRegion(): Map<Region, string[]> {
  const grouped = new Map<Region, string[]>();
  for (const code of ALL_LOCALES) {
    const region = LOCALE_REGIONS[code] || 'europe';
    if (!grouped.has(region)) {
      grouped.set(region, []);
    }
    grouped.get(region)!.push(code);
  }
  return grouped;
}
