...
//! v0.13.1 (ADR-030): Enhanced property handling with type info, required status, enum values.
//!
...
use crate::parsers::arcs::{ArcDef, ArcPropertyDef, ArcsDocument, Cardinality};
...
fn build_property_info_json(defs: &[ArcPropertyDef]) -> String {
    ...
    let props: Vec<String> = sorted
        ...
        .map(|p| {
            let mut parts = vec![
                format!("name: '{}'", p.name),
                format!("type: '{}'", p.prop_type),
                format!("required: {}", p.required),
            ];
            if let Some(ref enums) = p.enum_values {
                let enum_list: Vec<String> = enums.iter().map(|e| format!("'{}'", e)).collect();
                parts.push(format!("enum: [{}]", enum_list.join(", ")));
            }
...
/// Get ADR references relevant to an arc based on its properties and family.
fn get_adr_references(rel: &ArcDef) -> Vec<&'static str> {
    let mut refs = Vec::new();

    // Check for slug-related arcs (ADR-030)
    if rel.arc_type.contains("SLUG") || rel.arc_type == "DERIVED_SLUG_FROM" {
        refs.push("ADR-030");
    }

    // Check for TARGETS arc (ADR-030: is_slug_source property)
    if rel.arc_type == "TARGETS" {
        refs.push("ADR-030");
    }

    // Check for native arcs (ADR-029)
    if rel.arc_type.contains("NATIVE") {
        refs.push("ADR-029");
    }

    // Check for SEO/pillar arcs (ADR-031)
    if rel.arc_type.contains("SEO") || rel.arc_type.contains("CLUSTER") {
        refs.push("ADR-031");
    }
...
fn generate_arc_schema(
    ...
    cy!(out, "// ArcClass Schema-Nodes — v0.13.1 (ADR-029, ADR-030)");
    cy!(out, "// AUTO-GENERATED by novanet — DO NOT EDIT");
    cy!(
        out,
    ...
    for rel in &forward {
        ...
        if !adr_refs.is_empty() {
            cy!(out, "// {key} ({refs})", refs = adr_refs.join(", "));
        }

        cy!(out, "MERGE ({var}:Schema:ArcClass {{key: '{key}'}})");
        cy!(out, "ON CREATE SET");
        cy!(out, "  {var}.display_name = '{dn}',");
        cy!(out, "  {var}.content = '{content_val}',");
        cy!(out, "  {var}.family = '{family}',");
        if let Some(s) = scope {
            cy!(out, "  {var}.scope = '{s}',");
        } else {
            cy!(out, "  {var}.scope = null,");
        }
        cy!(out, "  {var}.cardinality = '{card}',");
        cy!(out, "  {var}.is_self_referential = {self_ref},");
        if let Some(inv_name) = inv {
            cy!(out, "  {var}.inverse_name = '{inv_name}',");
        } else {
            cy!(out, "  {var}.inverse_name = null,");
        }
        cy!(
            out,
        ...
        if let Some(ref info) = prop_info {
            ...
            cy!(out, "  {var}.arc_property_info = '{escaped}',");
        } else {
            cy!(out, "  {var}.arc_property_info = null,");
        }
        cy!(out, "  {var}.cypher_pattern = '{pattern}',");
        // Use actual temperature_threshold from arc-kind YAML if available
        if let Some(temp) = temps.get(key) {
            cy!(out, "  {var}.temperature_threshold = {temp},");
        } else {
            cy!(out, "  {var}.temperature_threshold = null,");
        }
        // (ADR-037): node_class discriminator (lowercase = SCHEMA node)
        cy!(out, "  {var}.node_class = 'arc_class',");
        cy!(out, "  {var}.created_by = 'seed:schema',");
    ...
    for rel in &forward {
        ...
        cy!(out, "MERGE (af)-[:HAS_ARC_CLASS]->(ac);");
        cy!(out);
    }
