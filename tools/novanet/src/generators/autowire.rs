...
fn generate_autowire(nodes: &[yaml_node::ParsedNode]) -> crate::Result<String> {
    ...
    cy!(out, "//");
    cy!(out, "// AUTO-GENERATED by novanet v0.13.0 — DO NOT EDIT");
    cy!(out, "// Total: {count} node types", count = nodes.len());
    cy!(
        out,
        "// ═══════════════════════════════════════════════════════════════════════════════"
    );
    ...
    for &realm_key in REALM_ORDER {
        ...
        cy!(out, "// {} REALM", realm_key.to_uppercase());
        cy!(
            out,
            "// ─────────────────────────────────────────────────────────────────────────────"
        );
        ...
        for (layer_key, node_names) in layers {
            ...
            for name in node_names {
                ...
                cy!(out, "MATCH (c:Class {{label: '{name}'}})");
                cy!(out, "MERGE (n)-[:OF_CLASS]->(c);");
                cy!(out);
            }
        }
    }
    ...
    cy!(out, "// VERIFICATION QUERY");
    cy!(
        out,
        "// ═══════════════════════════════════════════════════════════════════════════════"
    );
    cy!(out, "// Run this to verify the wiring was successful:");
    cy!(out, "//");
    ...
    cy!(out, "// OPTIONAL MATCH (c)<-[rel:OF_CLASS]-(n)");
    cy!(
        out,
        "// WITH r.key AS realm, l.key AS layer, c.label AS class, count(rel) AS instanceCount"
    );
    cy!(out, "// ORDER BY realm, layer, instanceCount DESC");
    cy!(out, "// RETURN realm, layer, class, instanceCount;");
...
mod tests {
    ...
    fn generate_autowire_integration() {
        ...
        // v0.20.0: 59 OF_CLASS statements
        let of_class = cypher
            .lines()
            .filter(|l: &&str| l.contains("MERGE") && l.contains("[:OF_CLASS]"))
            .count();
        assert_eq!(
            of_class, 59,
            "expected 59 OF_CLASS statements (36 shared + 23 org, v0.20.0)"
        );
        ...
        // v0.17.2: Layer counts (4 shared + 6 org = 10 layers)
        ...
        assert!(cypher.contains("Shared > Knowledge (21 types)")); // v0.18.0: 21 knowledge types
