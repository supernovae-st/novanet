...
fn generate_autowire(nodes: &[yaml_node::ParsedNode]) -> crate::Result<String> {
    ...
    .unwrap();
    writeln!(out, "//").unwrap();
    writeln!(out, "// AUTO-GENERATED by novanet v0.12.0 — DO NOT EDIT").unwrap();
...
mod tests {
    ...
    fn generate_autowire_integration() {
        ...
        // v0.12.4: 58 OF_CLASS statements (ADR-028: -2 deleted, +1 Country)
        let of_class = cypher
            .lines()
            .filter(|l: &&str| l.contains("MERGE") && l.contains("[:OF_CLASS]"))
            .count();
        assert_eq!(
            of_class, 58,
            "expected 58 OF_CLASS statements (v0.12.4: 40 shared + 18 org)"
        );
