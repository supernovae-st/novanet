...
fn generate_kind_cypher(nodes: &[ParsedNode]) -> crate::Result<String> {
    ...
    .unwrap();
    writeln!(out, "// Kind Meta-Nodes — v11.2.0").unwrap();
...
mod tests {
    ...
    fn context_budget_by_trait() {
        ...
        assert_eq!(
            context_budget(&make_node("PageGenerated", "org", "output", NodeTrait::Generated)),
            "low"
        );
        assert_eq!(
    ...
    fn visibility_kind_overrides() {
        ...
        assert_eq!(derive_visibility("org", "output", "PageGenerated"), "publishable");
        assert_eq!(derive_visibility("org", "output", "BlockGenerated"), "publishable");
        assert_eq!(derive_visibility("org", "semantic", "Entity"), "publishable");
        assert_eq!(derive_visibility("org", "semantic", "EntityContent"), "publishable");
    ...
    fn visibility_layer_rules() {
        ...
        assert_eq!(derive_visibility("shared", "config", "Locale"), "internal");
        assert_eq!(derive_visibility("shared", "locale-knowledge", "Term"), "internal");

        // Org publishable layers
        assert_eq!(derive_visibility("org", "semantic", "Unknown"), "publishable");
        assert_eq!(derive_visibility("org", "seo", "Unknown"), "publishable");
        assert_eq!(derive_visibility("org", "output", "Unknown"), "publishable");

        // Org fragment layers
        assert_eq!(derive_visibility("org", "foundation", "Project"), "fragment");
        assert_eq!(derive_visibility("org", "structure", "Unknown"), "fragment");

        // Org internal layers
        assert_eq!(derive_visibility("org", "config", "Org"), "internal");
        assert_eq!(derive_visibility("org", "instruction", "PagePrompt"), "internal");
    }
