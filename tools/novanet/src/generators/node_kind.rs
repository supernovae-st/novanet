...
fn generate_kind_cypher(nodes: &[ParsedNode]) -> crate::Result<String> {
    ...
    .unwrap();
    writeln!(out, "// Kind Meta-Nodes — v0.12.0").unwrap();
...
mod tests {
    ...
    fn generate_small_kind_cypher() {
        ...
        // EXHIBITS facet
        assert!(cypher.contains("(k:Kind {label: 'Page'}), (t:Trait {key: 'defined'})"));
        assert!(cypher.contains("MERGE (k)-[:EXHIBITS]->(t)"));
        assert!(cypher.contains("(k:Kind {label: 'Locale'}), (t:Trait {key: 'defined'})"));
    ...
    fn generate_kind_integration() {
        ...
        assert!(cypher.contains("(k:Kind {label: 'Page'}), (r:Realm {key: 'org'})"));
        assert!(cypher.contains("(k:Kind {label: 'Style'}), (t:Trait {key: 'imported'})")); // v10: LocaleVoice → Style
