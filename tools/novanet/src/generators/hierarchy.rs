...
mod tests {
    ...
    fn generate_hierarchy_integration() {
        ...
        assert!(output.contains("getNodeTypesByRealmAndLayer('shared', 'config')"));
        assert!(output.contains("getNodeTypesByRealmAndLayer('shared', 'locale-knowledge')"));
        ...
        assert!(output.contains("getNodeTypesByRealmAndLayer('org', 'seo')")); // v10.10: SEO moved to tenant
        ...
        assert!(output.contains("SHARED (2 layers)")); // config, locale-knowledge
        assert!(output.contains("ORG (7 layers)")); // config, semantic, seo, foundation, structure, instruction, output
