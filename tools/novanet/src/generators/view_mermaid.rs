...
fn render_view_mermaid(
    ...
    cy!(out, "flowchart TB");
    cy!(out, "  %% View: {} ({})", view.name, view.id);
    cy!(out, "  %% {} nodes, {} arcs", nodes.len(), arcs.len());
    cy!(out);
...
fn write_custom_layers(out: &mut String, nodes: &[&ParsedNode], layers: &[ViewLayer]) {
    ...
    for layer in layers {
        ...
        cy!(out, "  subgraph {layer_id}[\"{}\"]", layer.name);

        for node_name in &layer.nodes {
            if let Some(node) = node_map.get(node_name.as_str()) {
                let emoji = mermaid::layer_emoji(&node.def.layer);
                cy!(out, "    {node_name}[\"{emoji} {node_name}\"]");
    ...
    if !unassigned.is_empty() {
        cy!(out, "  %% Additional reachable nodes");
        for node in unassigned {
            let emoji = mermaid::layer_emoji(&node.def.layer);
            cy!(out, "  {}[\"{} {}\"]", node.def.name, emoji, node.def.name);
        }
        cy!(out);
    }
}
