...
mod tests {
    ...
    fn render_colors_basic() {
        ...
        assert!(output.contains("export type RealmKey = 'global' | 'tenant'"));
    ...
    fn render_colors_realm_entries() {
        ...
        assert!(output.contains("'tenant': 'Tenant'"));
    }
    ...
    fn generate_colors_integration() {
        ...
        // Should have all realms from YAML (v10.6: global + tenant)
        assert!(output.contains("'global'"));
        assert!(output.contains("'tenant'"));

        // Should have all 9 layers (v10.6: config now in both global and tenant, deduplicated)
        for layer in [
            "config",
            "locale-knowledge",
            "seo",
