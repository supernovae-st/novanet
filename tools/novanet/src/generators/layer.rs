...
mod tests {
    ...
    fn render_small_layers() {
        let nodes = vec![
            ...
            make_node("Style", "shared", "locale-knowledge"),
            make_node("SEOKeyword", "shared", "seo"),
            make_node("Organization", "org", "config"), // v10.6: org realm
        ];
        ...
        assert!(output.contains("// config (1 node)")); // shared/config
    ...
    fn generate_layers_integration() {
        ...
        // v11.2: 62 nodes (2 realms: shared + org, -3 job nodes)
        assert!(
            output.contains("mapping all 62 node types"),
            "should mention 62 node types"
        );

        // v11.2: Realm node counts (-3 job nodes from org)
        assert!(output.contains("SHARED REALM (32 nodes)")); // config (14) + locale-knowledge (18)
        assert!(output.contains("ORG REALM (30 nodes)")); // -3 job nodes (GenerationJob, SEOMiningRun, EvaluationSignal)
    ...
    fn snapshot_layers_typescript() {
        let nodes = vec![
            // Shared realm
            make_node("Locale", "shared", "config"),
            make_node("LocaleVoice", "shared", "locale-knowledge"),
            // Shared realm - SEO
            make_node("SEOKeyword", "shared", "seo"),
            make_node("SEOTarget", "shared", "seo"),
            // Org realm (v10.6: merged organization + project)
            make_node("Organization", "org", "config"),
