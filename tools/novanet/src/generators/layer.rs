...
mod tests {
    ...
    fn generate_layers_integration() {
        ...
        // v11.3: 61 nodes (2 realms: shared + org, merged Org+Tenant into OrgConfig)
        assert!(
            output.contains("mapping all 61 node types"),
            "should mention 61 node types"
        );

        // v11.3: Realm node counts
        assert!(output.contains("SHARED REALM (32 nodes)")); // locale (7) + geography (6) + knowledge (19)
        assert!(output.contains("ORG REALM (29 nodes)")); // v11.3: merged Org+Tenant into OrgConfig

        // 11 layers present (v11.3: 3 shared + 8 org)
        for layer in [
            "locale",
            "geography",
            "knowledge",
