...
//!
//! Reads all YAML node definitions and produces idempotent MERGE statements
...
fn generate_class_cypher(nodes: &[ParsedNode]) -> crate::Result<String> {
    ...
    for node in nodes {
        ...
        cy!(out, "MERGE ({var}:Schema:Class {{label: '{label}'}})");
        cy!(out, "ON CREATE SET");
        cy!(out, "  {var}.key = '{key}',");
        cy!(out, "  {var}.name = '{label}',");
        cy!(out, "  {var}.realm = '{realm}',");
        cy!(out, "  {var}.layer = '{layer}',");
        cy!(out, "  {var}.display_name = '{display}',");
        cy!(out, "  {var}.content = '{content_val}',");
        cy!(out, "  {var}.yaml_path = '{ypath}',");
        cy!(
            out,
        ...
        cy!(out, "  {var}.schema_hint = '{hint}',");
        cy!(out, "  {var}.context_budget = '{budget}',");
        cy!(out, "  {var}.visibility = '{visibility}',");
        // knowledge_tier for knowledge layer nodes only
        if let Some(tier) = &node.def.knowledge_tier {
            cy!(out, "  {var}.knowledge_tier = '{tier}',");
        }
        cy!(out, "  {var}.generation_count = 0,");
        // node_class discriminator (lowercase = SCHEMA node)
        cy!(out, "  {var}.node_class = 'class',");
...
mod tests {
    ...
    fn generate_class_integration() {
        ...
        assert_eq!(
            exhibits, 0,
            "expected 0 EXHIBITS relationships"
        );
