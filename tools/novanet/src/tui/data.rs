...
pub fn get_all_adrs() -> Vec<AdrEntry> {
    vec![
        ...
        AdrEntry {
            ...
            summary: vec![
                "Page ↔ Entity = N:M via [:ABOUT] (role + weight)".to_string(),
                "Slug = Entity.key (source of truth)".to_string(),
                "Order on arc: [:HAS_BLOCK {order}]".to_string(),
            ],
            diagram: vec![
                "Page ──[:ABOUT {role:'focus'}]──▶ Entity (N:M)".to_string(),
                "  │                        │".to_string(),
            ...
            key_rules: vec![
                "Every Page SHOULD have at least one ABOUT with role='focus'".to_string(),
