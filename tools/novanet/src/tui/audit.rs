...
/// Load detailed audit for a specific Kind (counts actual missing fields).
/// This is more expensive but gives accurate numbers for drill-down.
#[allow(dead_code)] // WIP: Audit mode implementation
pub async fn load_kind_audit_detail(
    db: &Db,
    kind_key: &str,
    required_props: &[String],
) -> crate::Result<KindAuditStats> {
    if required_props.is_empty() {
        // No required properties = always complete
...
"#,
        ...
        let rows = db.execute(&count_cypher).await?;
        let instance_count: i64 = rows
            .into_iter()
            .next()
            .and_then(|r| r.get("instance_count").ok())
            .unwrap_or(0);

        return Ok(KindAuditStats {
            kind_key: kind_key.to_string(),
            display_name: kind_key.to_string(),
            instance_count: instance_count as usize,
            total_properties: 0,
            complete_instances: instance_count as usize,
    ...
    // Build a Cypher query that checks each required property
    // This is a dynamic query based on the required properties
    let prop_checks: Vec<String> = required_props
        .iter()
        .map(|p| format!("(n.{} IS NULL OR n.{} = '')", p, p))
        .collect();

    let missing_condition = prop_checks.join(" OR ");

    let cypher = format!(
        r#"
MATCH (n:{kind_key})
WHERE NOT n:Meta
WITH n, CASE WHEN {missing_condition} THEN 1 ELSE 0 END AS has_missing
RETURN count(n) AS total,
       sum(has_missing) AS incomplete
"#,
        kind_key = kind_key,
    ...
    let rows = db.execute(&cypher).await?;
    let (total, incomplete) = rows
        .into_iter()
        .next()
        .map(|r| {
            let total: i64 = r.get("total").unwrap_or(0);
            let incomplete: i64 = r.get("incomplete").unwrap_or(0);
            (total as usize, incomplete as usize)
        })
        .unwrap_or((0, 0));

    let coverage_percent = if total > 0 {
        (((total - incomplete) * 100) / total) as u8
    } else {
        100
    };
    ...
    Ok(KindAuditStats {
        ...
        total_properties: required_props.len(),
        complete_instances: total.saturating_sub(incomplete),
        incomplete_instances: incomplete,
        coverage_percent,
        total_missing_required: incomplete * required_props.len(),
    })
}
