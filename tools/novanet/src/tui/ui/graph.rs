...
pub fn render_graph_panel(f: &mut Frame, area: Rect, app: &App) {
    ...
    if let Some(ref arcs) = app.kind_arcs {
        ...
        // Mini ASCII graph visualization
        render_mini_arc_graph(&mut lines, arcs, theme, &dim);
...
/// Render a mini ASCII graph showing the Kind in the center with
/// incoming sources on the left and outgoing targets on the right.
///
/// ```text
///     ┌──────────┐          ┌──────────┐          ┌──────────┐
///     │ Project  │──HAS───▶│   Page   │──USES───▶│  Entity  │
///     └──────────┘          └──────────┘          └──────────┘
/// ```
fn render_mini_arc_graph(
    lines: &mut Vec<Line>,
    arcs: &super::super::data::KindArcsData,
    theme: &theme::Theme,
    dim: &Style,
) {
    // Collect unique source and target kinds
    let incoming_kinds: Vec<&str> = arcs
        .incoming
        .iter()
        .map(|a| a.other_kind.as_str())
        .collect::<std::collections::HashSet<_>>()
        .into_iter()
        .take(3) // Limit to 3 for display
        .collect();

    let outgoing_kinds: Vec<&str> = arcs
        .outgoing
        .iter()
        .map(|a| a.other_kind.as_str())
        .collect::<std::collections::HashSet<_>>()
    ...
    if incoming_kinds.is_empty() && outgoing_kinds.is_empty() {
        return; // No arcs to display
    }

    let center_kind = &arcs.kind_label;
    let max_rows = incoming_kinds.len().max(outgoing_kinds.len()).max(1);
    ...
    let center_col_width = center_kind.len().max(10) + 2;

    // Helper to pad/truncate a name to fit in box
    let fit_name = |name: &str, width: usize| -> String {
        if name.len() > width {
    ...
    for row in 0..max_rows {
        let left_kind = incoming_kinds.get(row).copied();
        let right_kind = outgoing_kinds.get(row).copied();
        let is_center_row = row == max_rows / 2;

        // TOP BORDER ROW
        let mut top_spans: Vec<Span> = vec![Span::styled("  ", *dim)];

        // Left box top
        if left_kind.is_some() {
            top_spans.push(Span::styled(box_top_left, *dim));
            top_spans.push(Span::styled(box_horizontal.repeat(col_width), *dim));
            top_spans.push(Span::styled(box_top_right, *dim));
        } else {
            top_spans.push(Span::styled(" ".repeat(col_width + 2), *dim));
        }

        // Connector space
        top_spans.push(Span::styled("      ", *dim));

        // Center box top (only on first row)
        if row == 0 {
            top_spans.push(Span::styled(box_top_left, *dim));
            top_spans.push(Span::styled(box_horizontal.repeat(center_col_width), *dim));
            top_spans.push(Span::styled(box_top_right, *dim));
        } else {
            top_spans.push(Span::styled(" ".repeat(center_col_width + 2), *dim));
        }
        ...
        let mut mid_spans: Vec<Span> = vec![Span::styled("  ", *dim)];

        // Left box content
        if let Some(kind) = left_kind {
            mid_spans.push(Span::styled(box_vertical, *dim));
            mid_spans.push(Span::styled(
                fit_name(kind, col_width),
                Style::default().fg(theme.layer_color("semantic")),
            ));
            mid_spans.push(Span::styled(box_vertical, *dim));
        } else {
            mid_spans.push(Span::styled(" ".repeat(col_width + 2), *dim));
        }

        // Left connector (arrow pointing right)
        if left_kind.is_some() && is_center_row {
            mid_spans.push(Span::styled(
                format!("──{arrow_right}─", arrow_right = arrow_right),
                Style::default().fg(theme.arc_family_color("ownership")),
            ));
        } else if left_kind.is_some() {
            mid_spans.push(Span::styled("    ", *dim));
        } else {
            mid_spans.push(Span::styled("      ", *dim));
        }

        // Center box content (only on center row with content, else empty)
        if is_center_row || row == 0 {
            if is_center_row {
                mid_spans.push(Span::styled(box_vertical, *dim));
                mid_spans.push(Span::styled(
                    fit_name(center_kind, center_col_width),
                    Style::default()
                        .fg(Color::Green)
                        .add_modifier(Modifier::BOLD),
                ));
                ...
                mid_spans.push(Span::styled(box_vertical, *dim));
                mid_spans.push(Span::styled(" ".repeat(center_col_width), *dim));
                mid_spans.push(Span::styled(box_vertical, *dim));
            }
        } else {
            mid_spans.push(Span::styled(box_vertical, *dim));
        ...
        // Right connector (arrow pointing right)
        if right_kind.is_some() && is_center_row {
            mid_spans.push(Span::styled(
                format!("──{arrow_right}─", arrow_right = arrow_right),
                Style::default().fg(theme.arc_family_color("semantic")),
            ));
        } else if right_kind.is_some() {
            mid_spans.push(Span::styled("    ", *dim));
        } else {
            mid_spans.push(Span::styled("      ", *dim));
        }
        ...
        if let Some(kind) = right_kind {
            ...
            mid_spans.push(Span::styled(
                fit_name(kind, col_width),
                Style::default().fg(theme.layer_color("output")),
            ));
            mid_spans.push(Span::styled(box_vertical, *dim));
        }
        ...
        // BOTTOM BORDER ROW
        let mut bot_spans: Vec<Span> = vec![Span::styled("  ", *dim)];

        // Left box bottom
        if left_kind.is_some() {
            bot_spans.push(Span::styled(box_bottom_left, *dim));
            bot_spans.push(Span::styled(box_horizontal.repeat(col_width), *dim));
            bot_spans.push(Span::styled(box_bottom_right, *dim));
        } else {
            bot_spans.push(Span::styled(" ".repeat(col_width + 2), *dim));
        }

        // Connector space
        bot_spans.push(Span::styled("      ", *dim));

        // Center box bottom (only on last row)
        if row == max_rows - 1 {
            bot_spans.push(Span::styled(box_bottom_left, *dim));
            bot_spans.push(Span::styled(box_horizontal.repeat(center_col_width), *dim));
            bot_spans.push(Span::styled(box_bottom_right, *dim));
        } else {
            bot_spans.push(Span::styled(box_vertical, *dim));
            bot_spans.push(Span::styled(" ".repeat(center_col_width), *dim));
            bot_spans.push(Span::styled(box_vertical, *dim));
        }
    ...
    // Add counts indicator
    let incoming_extra = arcs.incoming.len().saturating_sub(3);
    let outgoing_extra = arcs.outgoing.len().saturating_sub(3);
    if incoming_extra > 0 || outgoing_extra > 0 {
        let mut counts = vec![Span::styled("  ", *dim)];
        if incoming_extra > 0 {
            counts.push(Span::styled(
                format!("  +{} more incoming", incoming_extra),
                STYLE_DIM,
            ));
        }
        if outgoing_extra > 0 {
            counts.push(Span::styled(
                format!("  +{} more outgoing", outgoing_extra),
                STYLE_DIM,
            ));
        }
...
mod tests {
    ...
    // =========================================================================
    // render_mini_arc_graph tests
    // =========================================================================

    #[test]
    fn test_render_mini_arc_graph_empty() {
        let theme = create_test_theme();
        let dim = Style::default().fg(Color::Rgb(100, 100, 100));
        let arcs = create_kind_arcs_data("Page", Vec::new(), Vec::new());
        ...
        // Should produce no lines when no arcs
        assert_eq!(lines.len(), 0, "empty arcs should produce 0 lines");
    }

    #[test]
    fn test_render_mini_arc_graph_outgoing_only() {
        let theme = create_test_theme();
        let dim = Style::default().fg(Color::Rgb(100, 100, 100));
        ...
        // Should have header + separator + box lines + blank line
        assert!(lines.len() >= 5, "should have graph lines");
        ...
        assert!(
            all_content.contains("ARC GRAPH"),
            "should contain ARC GRAPH header"
        );
        assert!(all_content.contains("Page"), "should contain center kind");
        assert!(all_content.contains("Entity"), "should contain target kind");
    }

    #[test]
    fn test_render_mini_arc_graph_incoming_only() {
        let theme = create_test_theme();
        let dim = Style::default().fg(Color::Rgb(100, 100, 100));

        let incoming = vec![create_neo4j_arc("BELONGS_TO", "Project", "ownership")];
        let arcs = create_kind_arcs_data("Page", incoming, Vec::new());
        ...
        assert!(
            ...
            "should contain source kind"
        );
    }

    #[test]
    fn test_render_mini_arc_graph_both_directions() {
        let theme = create_test_theme();
        let dim = Style::default().fg(Color::Rgb(100, 100, 100));
        ...
        // Should show all three kinds
        assert!(
            all_content.contains("Project"),
            "should contain incoming kind"
        );
        assert!(all_content.contains("Page"), "should contain center kind");
        assert!(
            all_content.contains("Entity"),
            "should contain outgoing kind"
        );
    }

    #[test]
    fn test_render_mini_arc_graph_limits_to_3() {
        let theme = create_test_theme();
        let dim = Style::default().fg(Color::Rgb(100, 100, 100));

        // Create 5 outgoing arcs
        let outgoing = vec![
            create_neo4j_arc("ARC1", "Kind1", "semantic"),
            create_neo4j_arc("ARC2", "Kind2", "semantic"),
            create_neo4j_arc("ARC3", "Kind3", "semantic"),
            create_neo4j_arc("ARC4", "Kind4", "semantic"),
            create_neo4j_arc("ARC5", "Kind5", "semantic"),
        ];
        let arcs = create_kind_arcs_data("Center", Vec::new(), outgoing);
        ...
        // Should show "+2 more outgoing" since we only display 3
        assert!(
            all_content.contains("+2 more outgoing"),
            "should indicate extra arcs not displayed, got: {}",
            all_content
        );
    }
