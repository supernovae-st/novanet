...
fn trait_llm_context(key: &str) -> &str {
    match key {
        ...
        "generated" => {
            "LLM-generated content output. Pages, blocks, and artifacts produced by the generation pipeline. Examples: PageNative, BlockNative, OutputArtifact."
        }
        "retrieved" => {
...
RETURN p.key, collect(b.key) AS blocks"#,
        ...
        "authored" => vec![
            CodeExample {
                title: "EntityNative (per-locale content)",
                yaml: r#"node:
  name: EntityNative
  realm: org
  layer: semantic
...
RETURN c.title, c.description"#,
  ...
  # Human-written per locale"#,
                neo4j: r#"(:ProjectNative {
  key: "project:qrcode-ai@fr-FR",
  tagline: "Créez des QR codes...",
  meta_description: "..."
})"#,
                cypher: r#"MATCH (p:Project)-[:HAS_NATIVE]->(c:ProjectNative)
WHERE c.key ENDS WITH $locale
RETURN c.tagline"#,
            },
        ],
...
RETURN e.pattern"#,
  ...
  generated_at: datetime()
            ...
            cypher: r#"MATCH (p:Page)-[:HAS_NATIVE]->(g:PageNative)
...
mod tests {
    ...
    fn test_code_examples_authored() {
        ...
        assert_eq!(examples.len(), 2);
        assert!(examples[0].title.contains("EntityNative"));
    ...
    fn test_code_examples_generated() {
        ...
        assert!(examples[0].title.contains("PageNative"));
