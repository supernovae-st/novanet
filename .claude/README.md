...
**Version**: v11.3.0 | **Docs**: [Claude Code Official](https://docs.anthropic.com/en/docs/claude-code)
...
**Source of truth:** `/VERSION` file contains the canonical schema version (11.3.0).

**`pnpm doc:audit` checks:**
- Outdated version references (current: v11.3.0)
...
- Incorrect node/arc counts (61 nodes, 125 arcs expected)
- Outdated realm names (global → shared, tenant → org)
- Outdated layer names (locale-knowledge → locale, geography, knowledge)

**`pnpm skill:audit` checks:**
- Deprecated paths (`nodes/` → `node-kinds/`, `organizing-principles.yaml` → `taxonomy.yaml`)
- Node/arc counts match YAML sources (61 node-kinds, 125 arc-kinds)
