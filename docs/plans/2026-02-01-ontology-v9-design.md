...
`core/src/types/nodes.ts` currently has hardcoded constants:
- `NODE_TYPES` (35 entries)
- `NODE_CATEGORIES` (6 categories)
- `NODE_BEHAVIORS` (35 mappings)
- `NODE_SCOPES` (35 mappings)

v9 approach: **generate from YAML** instead of hardcoding.
The type names and exports can stay the same to minimize import breakage:
...
// Backward-compatible exports (same names, now generated)
export const NODE_TYPES = [...] as const;
export const NODE_SCOPES: Record<NodeType, Realm> = { ... };
export const NODE_BEHAVIORS: Record<NodeType, Trait> = { ... };
// NODE_CATEGORIES replaced by Layer-based grouping
```
...
Total: ~119 files across 5 packages + 1 Rust tool + docs + Claude config. Grouped by package.
...
| `src/graph/hierarchy.ts` | Update | Scope→Realm, Subcategory→Layer in hierarchy logic |
| `src/graph/generator.ts` | Update | Update for new meta-node types |
| `src/filters/types.ts` | Update | `NodeCategory` (6→9 Layers), `ViewCategory` scope→realm |
| `src/filters/CypherGenerator.ts` | Update | `NodeCategory` expansion logic → Layer-based |
| `src/filters/NovaNetFilter.ts` | Update | `NodeCategory` filtering → Layer-based |
...
| `src/config/nodeTypes.ts` | Align categories | 6 visual → 9 Layers from meta-graph |
| `src/lib/filterAdapter.ts` | Update | `NodeCategory` (6→9 Layers) + category groupings |
...
| `src/config/viewCategories.ts` | Update | Scope→Realm icon mapping |
