...
| `KindGenerator` | 35 node YAML files | Kind nodes (+ `schema_hint`, `context_budget`) + IN_REALM, IN_LAYER, EXHIBITS facettes |
| `EdgeSchemaGenerator` | `relations.yaml` | EdgeKind nodes (+ `cypher_pattern`) + FROM_KIND, TO_KIND, IN_FAMILY, HAS_EDGE_KIND |
