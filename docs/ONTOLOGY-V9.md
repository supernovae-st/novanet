...
NovaNet is a **native content generation system** powered by a Neo4j knowledge graph.
It orchestrates the generation of locale-specific content across 200+ locales for
...
inspired by OWL (Web Ontology Language). Every node type sits at the intersection
of multiple classification axes, and every relationship type is a first-class citizen
with typed source/target constraints.
...
4. Rewrite generators (split into 6)
...
`core/src/types/nodes.ts` hardcoded constants become generated:
...
~119 files across 5 packages + 1 Rust tool + turbo generators + docs + Claude config. See the full detailed inventory in
...
|---------|-------|-------------|
| `@novanet/core` | ~23 | YAML migration (35 nodes + relations + organizing-principles), TypeScript types, graph module, filters (NodeCategory→Layer) |
| `@novanet/schema-tools` | ~11 | 6 generators (split from 1 + 3 new), RelationsParser, colors, tests |
| `@novanet/db` | ~8 | Constraints, seeds, autowire, queries audit |
| `@novanet/studio` | ~43 | Component renames (SchemaNode, tokens), API queries, hooks, stores, filters, 6 layout algorithms, filterAdapter, tailwind, ELK test, e2e tests, navigation modes (5 new) |
