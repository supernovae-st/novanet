...
flowchart TB
    subgraph MONO["NovaNet Monorepo"]
        direction TB
        CORE["@novanet/core v0.15.1\nTypes · Schemas · Filters"]
        DB["@novanet/db v0.15.1\nDocker · Seeds · Migrations"]
        STUDIO["@novanet/studio v0.15.1\nNext.js 16 · React 19"]
        RUST["novanet CLI v0.15.1\nRust · 16 commands · TUI"]
    end
...
|---------|---------|-------------|
| **@novanet/core** | `0.15.1` | Types, Zod schemas, NovaNetFilter API, Cypher generators |
| **@novanet/db** | `0.15.1` | Docker Compose for Neo4j, Cypher seeds, migrations |
| **@novanet/studio** | `0.15.1` | Interactive graph visualization with AI chat |
| **tools/novanet** | `0.15.1` | Rust CLI + TUI for schema generation, validation, queries |
| **tools/novanet-mcp** | `0.15.1` | MCP Server for AI agent integration (11 tools) |
...
NovaNet models content as a knowledge graph with **2 Realms** and **10 Layers** (v0.15.1):
...
> **v0.15.1 ADR-024: Data Origin traits** — Trait now answers "WHERE does data come from?" (defined = human-created once, authored = human-written per locale, imported = external data brought in, generated = LLM output, retrieved = external API snapshots)
