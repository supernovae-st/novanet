...
ON CREATE SET
  ...
  l_config.llm_context = 'Invariant definitions for shared realm. All nodes have invariant trait. v11.5: Locale (BCP-47 codes), EntityCategory (v11.1, ADR-017), SEOKeywordFormat (v11.4). These are universal DEFINITIONS, not locale-specific parameters.',
...
ON CREATE SET
  ...
  l_locale.llm_context = 'Locale-specific SETTINGS (1:1 with Locale). All nodes have knowledge trait. v11.5: Culture, Style, Formatting, Adaptation, Market, Slugification. These contain discovered/curated data specific to each locale. Pattern: Locale (config) --[:HAS_X]--> Setting (locale).',
