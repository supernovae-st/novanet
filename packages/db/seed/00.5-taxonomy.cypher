...
ON CREATE SET
  ...
  r_org.emoji = '🏢',
  r_org.color = '#6c71c4',
  r_org.llm_context = 'Organization-specific realm. Contains Projects, Entities, Pages, Blocks, and all generated outputs. Org A cannot see Org B (multi-tenant isolation). Organization owns Entity directly (HAS_ENTITY). Page/Block reference Entity via USES_ENTITY (N:M). AI context assembly follows arcs from starting point via spreading activation.',
...
MATCH (r:Realm {key: 'shared'}), (l:Layer {key: 'locale-knowledge'})
MERGE (r)-[:HAS_LAYER]->(l);
