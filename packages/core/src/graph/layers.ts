// packages/core/src/graph/layers.ts
// NODE_LAYERS mapping all 59 node types to their layers
// AUTO-GENERATED by novanet v0.13.0 — DO NOT EDIT
// Run: novanet schema generate

import type { NodeType, Realm } from '../types/nodes.js';
import type { Layer } from './types.js';
import { CLASS_TAXONOMY } from '../types/nodes.js';

// =============================================================================
// NODE_LAYERS - Maps each NodeType to its layer
// =============================================================================

/**
 * Maps each NodeType to its layer within its realm.
 * AUTO-GENERATED from models/node-classes/ folder structure.
 *
 * Layers by realm:
 * - Shared: config, geography, knowledge, locale
 * - Org: config, foundation, instruction, output, semantic, structure
 */
export const NODE_LAYERS: Record<NodeType, Layer> = {
  // ═══════════════════════════════════════════════════════════════════════════
  // SHARED REALM (36 nodes)
  // ═══════════════════════════════════════════════════════════════════════════

  // config (3 nodes) - matches models/node-classes/shared/config/
  EntityCategory: 'config',
  Locale: 'config',
  SEOKeywordFormat: 'config',

  // geography (7 nodes) - matches models/node-classes/shared/geography/
  Continent: 'geography',
  Country: 'geography',
  EconomicRegion: 'geography',
  GeoRegion: 'geography',
  GeoSubRegion: 'geography',
  IncomeGroup: 'geography',
  LendingCategory: 'geography',

  // knowledge (21 nodes) - matches models/node-classes/shared/knowledge/
  AudienceSet: 'knowledge',
  AudienceTrait: 'knowledge',
  CulturalRealm: 'knowledge',
  CulturalSubRealm: 'knowledge',
  CultureRef: 'knowledge',
  CultureSet: 'knowledge',
  Expression: 'knowledge',
  ExpressionSet: 'knowledge',
  GEOAnswer: 'knowledge',
  GEOQuery: 'knowledge',
  GEOQuerySet: 'knowledge',
  LanguageBranch: 'knowledge',
  LanguageFamily: 'knowledge',
  Pattern: 'knowledge',
  PatternSet: 'knowledge',
  PopulationCluster: 'knowledge',
  PopulationSubCluster: 'knowledge',
  SEOKeyword: 'knowledge',
  SEOKeywordSet: 'knowledge',
  Taboo: 'knowledge',
  TabooSet: 'knowledge',

  // locale (5 nodes) - matches models/node-classes/shared/locale/
  Adaptation: 'locale',
  Culture: 'locale',
  Formatting: 'locale',
  Slugification: 'locale',
  Style: 'locale',

  // ═══════════════════════════════════════════════════════════════════════════
  // ORG REALM (23 nodes)
  // ═══════════════════════════════════════════════════════════════════════════

  // config (1 node) - matches models/node-classes/org/config/
  OrgConfig: 'config',

  // foundation (8 nodes) - matches models/node-classes/org/foundation/
  Brand: 'foundation',
  BrandDesign: 'foundation',
  BrandPrinciples: 'foundation',
  Project: 'foundation',
  ProjectGEOScope: 'foundation',
  ProjectNative: 'foundation',
  ProjectSEOScope: 'foundation',
  PromptStyle: 'foundation',

  // instruction (3 nodes) - matches models/node-classes/org/instruction/
  BlockInstruction: 'instruction',
  BlockType: 'instruction',
  PromptArtifact: 'instruction',

  // output (6 nodes) - matches models/node-classes/org/output/
  BlockNative: 'output',
  CultureRefEnrichment: 'output',
  ExpressionEnrichment: 'output',
  OutputArtifact: 'output',
  PageNative: 'output',
  TabooEnrichment: 'output',

  // semantic (2 nodes) - matches models/node-classes/org/semantic/
  Entity: 'semantic',
  EntityNative: 'semantic',

  // structure (3 nodes) - matches models/node-classes/org/structure/
  Block: 'structure',
  ContentSlot: 'structure',
  Page: 'structure',

};

// =============================================================================
// HELPER FUNCTIONS
// =============================================================================

/**
 * Get the layer for a node type
 * @param nodeType - The node type to look up
 * @returns The layer for the node type
 */
export function getLayer(nodeType: NodeType): Layer {
  return NODE_LAYERS[nodeType];
}

/**
 * Get all node types in a layer
 * @param layer - The layer to look up
 * @returns Array of node types belonging to the layer
 */
export function getNodeTypesByLayer(layer: Layer): NodeType[] {
  return (Object.entries(NODE_LAYERS) as [NodeType, Layer][])
    .filter(([, l]) => l === layer)
    .map(([nodeType]) => nodeType);
}

/**
 * Get all node types in a specific realm AND layer
 * @param realm - The realm to filter by
 * @param layer - The layer to filter by
 * @returns Array of node types belonging to both the realm and layer
 */
export function getNodeTypesByRealmAndLayer(realm: Realm, layer: Layer): NodeType[] {
  return (Object.entries(CLASS_TAXONOMY) as [NodeType, { realm: Realm; layer: Layer }][])
    .filter(([, meta]) => meta.realm === realm && meta.layer === layer)
    .map(([nodeType]) => nodeType);
}